...
class ITArchitectsSuiteEnhanced {
    ...
    async generateArchitecture(prompt, analysis) {
        try {
            ...
            const response = await fetch(`${this.config.apiBaseUrl}/ai/generate-architecture`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    prompt,
    ...
    async loadDiagramInDrawio(xmlData) {
        try {
            ...
            // Configure embed.diagrams.net iframe with proper XML data handling
            // Parameters: embed=1 (embedded mode), ui=dark (dark theme), spin=1 (loading spinner)
            // chrome=0 (minimal chrome), modified=unsavedChanges (change tracking), proto=json (JSON protocol)
            ...
            console.log('Diagram loaded in embed.diagrams.net iframe');
        } catch (error) {
    ...
    openDrawioEditor(xmlData) {
        try {
            // Use embed.diagrams.net for editor mode with XML data in URL
            const encodedXML = encodeURIComponent(xmlData);
            const editUrl = `${this.config.drawioBaseUrl}/?embed=1&ui=dark&spin=1&chrome=0&proto=json&xml=${encodedXML}`;
            const editorWindow = window.open(editUrl, 'drawio-editor', 'width=1200,height=800,scrollbars=yes,resizable=yes');
            
            // Listen for changes from draw.io editor
            this.setupDrawioMessageListener(editorWindow);
            
            console.log('Opened embed.diagrams.net editor in new window');
        } catch (error) {
    ...
    // Setup iframe message listener for embed.diagrams.net
    setupDrawioIframeListener(iframe) {
            ...
            if (event.source === iframe.contentWindow) {
                try {
                    ...
                    if (data.event === 'save' || data.event === 'autosave') {
                        console.log('Received diagram update from embed.diagrams.net iframe');
                        this.handleDiagramUpdate(data.xml);
                    }
                    
                    if (data.event === 'configure') {
                        // Configure the embedded editor
                        const configMessage = {
                            action: 'configure',
                            config: {
                                defaultLibraries: 'general;uml;er;aws4;azure;gcp2',
                                autosave: true,
                                compressXml: true
                            }
                        };
                    ...
                    console.error('Error processing embed.diagrams.net iframe message:', error);
                }
            }
        });
    }
